...
<body style=" overflow: hidden;">
    <div style=" height: 65px; overflow: hidden;">
        <span style="margin:0px 20px 0px; font-size: 14px; line-height: 65px;  color: #888;">当前公告总数{{$sum->total}}条</span>
    ...
    <ul style="overflow: hidden;">
        {{foreach from=$postinfo item=value}}
        <li style="border-bottom: 1px solid #ccc; overflow: hidden; height:50px;">
            <span style="display:block;float:left;width:28%;font-size:14px;">
                <span style="line-height: 50px; margin-left: 20px;">{{$value->post_title}}</span>
            </span>
            <span style=" display:block;float:left;width:22%;font-size:14px;">
                     <span style="line-height: 50px;">{{$value->post_type}}</span>
            </span>
            <span style=" display:block;float:left;width:25%;font-size:14px;">
                     <span style="line-height: 50px;">{{$value->post_date}}</span>
            </span>
            <span style=" display:block;float:left;width:20%;font-size:14px;">
                <a href="admin.php?c=BackplanControl&m=post_content&id={{$value->id}}" style="text-decoration: none;line-height: 50px;">修改</a>
                <a href="admin.php?c=BackplanControl&m=postDelete&id={{$value->id}}" style="text-decoration: none;line-height: 50px;" onclick="return confirm('你真的要删除这个管理员吗？') ? true : false">删除</a>
            </span>

        </li>
        {{/foreach}}
    </ul>
    <center style="margin-top: 20px;">{{$page_html}}<center>
</body>
</html>